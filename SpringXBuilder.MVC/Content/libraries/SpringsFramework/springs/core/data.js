...
sp.core.data.DataUtils =
{
    ...
    {
        if(val && val.toLowerCase) val = val.toLowerCase();
        strVals = strVals || [];
    ...
    valueAtPath: function(obj, path)
    {
        ...
        if (parts.length > 1)
        {
            ...
            var arraySearch = prop.match(/\[(\w+)=(\w+)\]/);
            if (arraySearch)
            {
                var arrayProp = arraySearch[1];
                var arrayValue = arraySearch[2];

                var item = sp.utils.ArrayUtils.findElementByProperty(obj, arrayProp, arrayValue);
                return this.valueAtPath(item, parts.join("."));
            }
        ...
        {
            var numberedDate = sp.core.date.Date.ReturnDateStringAsNumericValue(obj);
            return numberedDate.toString();
        }
        else
        {
            return obj[path];
    ...
    setValueAtPath: function(obj, path, value)
    {
        ...
        if (path.length > 1)
        {
            ...
            if (arraySearch)
            {
                ...
                {
                    item = {};
                    item[arrayProp] = arrayValue;
                    obj.push(item);
                }

                this.setValueAtPath(item, path.join('.'), value);
            }
            else
            {
                if (typeof(obj[prop]) != 'object') obj[prop] = {};
                this.setValueAtPath(obj[prop], path.join('.'), value);
            }
        }
        else
        {
...
sp.core.data.RadioGroupDataOptions = sp.core.data.ValueObject.extend
(
    {
        __constructor: function(valueMap)
        {
...
sp.core.data.RadioGroupData = sp.core.events.EventDispatcher.extend
(
    {
        __constructor: function(data, options)
        {
            this.__super();
            this.data = sp.core.data.DataUtils.toArray(data);
            this.options = options || new sp.core.data.RadioGroupDataOptions();
        },
        ...
        getHTML: function(groupID,options)
        {
            var html = "";
            for (var i = 0; i < this.data.length; i++)
            {
                html += '<label class="radio">';
                html += '<input type="radio" name="' + groupID + '" value="' + this.data[i][this.options.idProp] +'"/>';
                html += '<span>' + this.options.getLabel(this.data[i]) + '</span>';
                html += '</label>';
            }
            return html;
        },
