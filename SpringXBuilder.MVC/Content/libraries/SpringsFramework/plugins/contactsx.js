 ...
 */
sp.namespace("plugins.contactsx.ContactData",
    "plugins.contactsx.ContactData",
    "plugins.contactsx.ContactList",
    "plugins.contactsx.ContactTypes");

plugins.contactsx.ContactData = sp.core.events.EventDispatcher.extend
(
    {
        __constructor:function(xml)
        {
            ...
            {
                var contact = new plugins.contactsx.Contact(childNodes[i]);
            ...
            plugins.contactsx.ContactData.__INSTANCE = this;
        },
        ...
        {
            var contact = new plugins.contactsx.Contact();
        ...
        addContact:function(contact)
        {
            ...
            if(!this.data[contact.getID()]) this.data[contact.getID()] = contact;
...
plugins.contactsx.ContactData.getInstance = function()
{
    if(!plugins.contactsx.ContactData.__INSTANCE) plugins.contactsx.ContactData.__INSTANCE = new plugins.contactsx.ContactData();
    return plugins.contactsx.ContactData.__INSTANCE;
};

plugins.contactsx.Contact = spx.model.DataWrapper.extend
(
    {
        __constructor:function(xml)
        {
            ...
            if(!this.data.type) this.data.type = plugins.contactsx.ContactTypes.EXTERNAL;
        },
...
plugins.contactsx.ContactSearchDialog = sp.ui.dialogs.Dialog.extend
(
    {
        ...
        init: function()
        {
            ...
            options.props = ["first_name", "last_name", "title", "account", "location"];
            options.labels = [
        ...
        onBeforeOK: function()
        {
            ...
            var contact = new plugins.contactsx.Contact(null);
            contact.data = c;
            var XMLUtils = sp.utils.XMLUtils;
            for (var prop in contact.data)
            {
                contact.data[prop] = XMLUtils.toNumberOfHtmlEscapes(c.ContactName, 0);
            }
            plugins.contactsx.ContactData.getInstance().addContact(contact);
        },
        ...
        startSearch: function()
        {
            ...
            var url = this.appletParams.CONTACTSEARCH + '?search=' + search + '&type=' + type;
            if (lastname) url += '&lastname=' + lastname;
            if (account) url += '&accountname=' + account;
            ...
            if(!this.searchRecordset)
            {
                var xml = sp.utils.XMLUtils.strToXML(
                    "<recordset id='contacts'>" +
                        ...
                        "</record>" +
                    "</recordset>");
                this.searchRecordset = new spx.model.datastructure.RecordSet(xml);
            }
            return this.searchRecordset;
        },
        ...
        onSearchResult: function(event)
        {
            ...
            if (event.success)
            {
                ...
                this.getSearchRecord().setData( spx.model.datalayer.FRRSLayer.convert($(event.target.data).find("#contacts")[0]));

                var tableData = $(this.getSearchRecord().getElements()).map(function(index, element){ return element.data });
                this.table.setDataProvider(tableData);
...
plugins.contactsx.ContactDataEvent = sp.core.events.Event.extend
(
    {
...
plugins.contactsx.ContactDataEvent.CONTACT_ADDED = "contact_added";
plugins.contactsx.ContactDataEvent.CONTACT_REMOVED = "contact_removed";

plugins.contactsx.ContactSelector = spx.controller.ComboController.extend
(
    {
        __init:function()
        {
            this.options = spx.model.resources.Resources.getResourceByID(this.data.getAttribute("options")) || new plugins.contactsx.ContactSelectorOptions();
            this.details = (this.data.getAttribute('details') || 'name').split(',');
            plugins.contactsx.ContactData.getInstance().addEventListener(this, sp.core.data.DataEvent.CHANGE, this.onContactDataChange);
            this.__super();
        },
        ...
        onChange:function()
        {
            if(this.selection.getSelectedValue()=="SEARCH")
            {
                ...
                var contactSearchDialog = new plugins.contactsx.ContactSearchDialog(spx.AppletParameters, this.options);
        ...
        onDialog: function(event)
        {
            if (event.result == sp.ui.dialogs.Dialog.OK)
            {
                ...
                {
                    this.model.setData(event.target.getSelectedContact().id);
                }
                else
                {
                    this.setData(event.target.getSelectedContact().id);
                }
        ...
        getSelectionData:function()
        {
            ...
            var selectionData = plugins.contactsx.ContactData.getInstance().getContactSelection(filter);
            var __this = this;
...
plugins.contactsx.ContactSelectorOptions = spx.model.DataWrapper.extend
(
    {
        __constructor:function(xml)
        {
...
plugins.contactsx.ContactSelectorEvent = sp.core.events.Event.extend({});
plugins.contactsx.ContactSelectorEvent.CONTACT_SELECTED = "contact_selector_select";
