...
(function($) {
    ...
    $.jqplot.BubbleRenderer.prototype.init = function(options, plot) {
        ...
        function adjustLabels() {
            ...
            var labelPlaces = {};
            for (var i=0,len=firstSeries.labels.length; i<len; i++) {
                var currentLabel = firstSeries.labels[i];
                var currentLabelYOffset = Math.ceil(parseInt(currentLabel.css('top')) / 20) * 20;
                var currentLabelXOffset = Math.ceil((parseInt(currentLabel.css('left'))+(currentLabel.width()/2)) / 20) * 20;
                var key = currentLabelXOffset + ':' + currentLabelYOffset;
                var theArray = labelPlaces[key] || (labelPlaces[key] = []);

                theArray.push(currentLabel);
                //console.log(currentLabel.text() + ': ' +key)
            }
            for (var key in labelPlaces) {
                if(labelPlaces.hasOwnProperty(key)) {
                    if (labelPlaces[key].length > 1) {
                        var adjust = labelPlaces[key].length;
                        var currentY = parseInt(key.split(':')[1])-20;
                        for (var i=0,len=labelPlaces[key].length; i<len; i++) {
                            labelPlaces[key][i].css('top', currentY+'px');
                            labelPlaces[key][i].css('zIndex', 100);
        ...
        plot.postDrawHooks.add(adjustLabels);
        plot.target.on('jqplotDragStop', function () {adjustLabels.apply(plot)})
        plot.target.on('jqplotSeriesPointChange', function () {adjustLabels.apply(plot)})
    };
